...
const tourModel = {
    ...
    async getAllToursModel(){
        try {
            const peticion = await fetch(process.env.BDD_TOURS)
    ...
    async getTourByIdModel(tourId) {
        const response = await fetch(`${process.env.BDD_TOURS}/${tourId}`);
        if (!response.ok) {
    ...
    async createTourModel(newTour){
        // 1.- CONEXION BDD
        const url = process.env.BDD_TOURS
    ...
    async updateTourModel(tourId,updatedTour){
           ...
           // 1.- CONEXION BDD
        const url = `${process.env.BDD_TOURS}/${tourId}`
        // 2.- ENVIAR DATA A LA BDD
        const peticion = await fetch(url,{
            method:"PUT",
